...
module Crisp
  class WebsiteResource
    ...
    LIST_ANALYTICS_CLASSIFIERS_QUERY_PARAMETERS = [
      "date_from",
      "date_to",
    ]
    ...
    def list_analytics_classifiers(website_id, type, metric, page_number = 1, date_from = "", date_to = "")
      resource_url = ""
      query_parameters = []

      LIST_ANALYTICS_CLASSIFIERS_QUERY_PARAMETERS.each do |parameter|
        parameter_value = binding.local_variable_get(parameter)
      ...
      if query_parameters != []
        resource_url = self._url_website(website_id, "/analytics/%s/%s/classifiers/%d?%s" % [type, metric, page_number, query_parameters.join("&")])
      else
        resource_url = self._url_website(website_id, "/analytics/%s/%s/classifiers/%d" % [type, metric, page_number])
      end
